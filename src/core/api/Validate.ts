    ...
    descriptor.value = async function(...args: any[]): Promise<any> {
        ...
        if (requestParameters.length > 0) {
            for (const requestParameter of requestParameters) {
                ...
                await requestBody.customObjectValidate(false, requestParameter.request);
            }
        }
...
/**
 * Validate annotation builds the given RequestBodies and validates them assuming that the request is an array
 *
 * @param target
 * @param propertyName
 * @param descriptor
 */
export const validateArray = () => (target: any, propertyName: string, descriptor: TypedPropertyDescriptor<any>): any => {
    const method = descriptor.value;
    descriptor.value = async function(...args: any[]): Promise<any> {
        const requestParameters: RequestParameter[] = Reflect.getOwnMetadata(requestMetadataKey, target, propertyName);
        if (requestParameters.length > 0) {
            for (const requestParameter of requestParameters) {
                const requestBody = new requestParameter.request(args[requestParameter.index]);
                await requestBody.batchValidate(false, requestParameter.request);
            }
        }
        return method && method.apply(this, args);
    };
